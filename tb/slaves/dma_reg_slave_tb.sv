...
module dma_reg_slave_tb;
  ...
  localparam int AXI_TIMEOUT_CYCLES = 50;
  ...
  task automatic axi_write(
    ...
    begin
      ...
      do begin
        @(posedge clk);
        timeout_count++;
      end while (!intf.aw_ready && timeout_count < AXI_TIMEOUT_CYCLES);

      if (!intf.aw_ready) begin
        $error("AXI WRITE TIMEOUT: AWREADY not seen addr=%h", addr);
        error_count++;
        intf.aw_valid <= 1'b0;
        return;
      end
      ...
      do begin
        @(posedge clk);
        timeout_count++;
      end while (!intf.w_ready && timeout_count < AXI_TIMEOUT_CYCLES);

      if (!intf.w_ready) begin
        $error("AXI WRITE TIMEOUT: WREADY not seen addr=%h", addr);
        error_count++;
        intf.w_valid <= 1'b0;
        intf.w.last  <= 1'b0;
        return;
      ...
      do begin
        @(posedge clk);
        timeout_count++;
      end while (!intf.b_valid && timeout_count < AXI_TIMEOUT_CYCLES);

      if (!intf.b_valid) begin
        $error("AXI WRITE TIMEOUT: BVALID not seen addr=%h", addr);
        error_count++;
        intf.b_ready <= 1'b0;
        return;
      end
  ...
  task automatic axi_read_check(
    ...
    begin
      ...
      do begin
        @(posedge clk);
        timeout_count++;
      end while (!intf.ar_ready && timeout_count < AXI_TIMEOUT_CYCLES);

      if (!intf.ar_ready) begin
        $error("%s TIMEOUT: ARREADY not seen addr=%h", msg, addr);
        error_count++;
        intf.ar_valid <= 1'b0;
        return;
      end
      ...
      do begin
        @(posedge clk);
        timeout_count++;
      end while (!intf.r_valid && timeout_count < AXI_TIMEOUT_CYCLES);

      if (!intf.r_valid) begin
        $error("%s TIMEOUT: RVALID not seen addr=%h", msg, addr);
        error_count++;
        intf.r_ready <= 1'b0;
        return;
      end
  ...
  task automatic config_register_test;
    begin
      ...
      axi_write(REG_LENGTH, 32'h1234_5678);
      axi_read_check(REG_LENGTH, 32'h1234_5678, "LENGTH full readback");

      if (dma_length !== 16'h5678) begin
        $error("OUTPUT ERROR: dma_length expected lower 16 bits 5678 got=%h", dma_length);
        error_count++;
      end
  ...
  begin
    ...
    else if (start_pulse_count != 1) begin
      $error("START PULSE ERROR: expected exactly 1 pulse, observed %0d", start_pulse_count);
      error_count++;
    end
    else begin
      $display("START PULSE ONE-CYCLE OBSERVED PASS");
    end
...
endtask
  ...
  task automatic status_error_sticky_test;
    begin
      ...
      axi_read_check(REG_STATUS, expected_status(1'b1, 1'b0, 1'b1), "STATUS done/error sticky before start");
      axi_write(REG_CONTROL, 32'h0000_0001);
      axi_read_check(REG_STATUS, expected_status(1'b0, 1'b0, 1'b0), "STATUS cleared by CONTROL start");
  ...
  task automatic invalid_address_test;
    begin
      ...
      axi_read_check(REG_SRC_ADDR, 32'h0000_0040, "SRC unchanged after invalid write");
      axi_read_check(REG_DST_ADDR, 32'h0000_0180, "DST unchanged after invalid write");
      axi_read_check(REG_LENGTH,   32'h0000_0010, "LENGTH unchanged after invalid write");
